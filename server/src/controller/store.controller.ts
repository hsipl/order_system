...
import { StoreParam } from "../interafaces/api/repository/store.interface";
...
class StoreController {
    ...
    async create(req: Request, res: Response, next: NextFunction) {
        ...
        if (!name || status === undefined || type === undefined) {
            return next(new ErrorHandler(errorStatusCode.badRequest, errorMsg.storeParameterError))
        }
        try {
            const storeExist = await this.service.getByName(name)
            if (storeExist) {
